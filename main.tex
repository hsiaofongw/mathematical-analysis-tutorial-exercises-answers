...
\end{equation}
这样我们就找到了一个趋于$l$的子列，它就是$\{ a_{j_m} \}$．如果$u = m$，也就是$\{ a_n \}$的上确界等于下确界，那么将可以推出$\{ a_n \}$是常数列，这时每一个$a_n$都可以当成最大值（最小值），这与题设矛盾，所以$u \neq l$，所以$\{ a_{i_n} \}$和$\{ a_{j_m} \}$是$\{ a_n \}$的两个趋向不同极限的子列，所以$\{ a_n \}$发散．
\end{proof}
